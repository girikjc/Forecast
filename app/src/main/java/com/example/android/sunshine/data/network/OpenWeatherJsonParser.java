...
final class OpenWeatherJsonParser {
    ...
    private static final String OWM_WEATHER_WIND = "wind";
    ...
    private static WeatherEntry[] fromJson(final JSONObject forecastJson) throws JSONException {
        ...
        System.out.println("length : " + jsonWeatherArray.length());
        /*
    ...
    private static WeatherEntry fromJson(final JSONObject dayForecast,
        ...
        //First get main and wind
        JSONObject main = dayForecast.getJSONObject(OWM_WEATHER_MAIN);
        JSONObject wind = dayForecast.getJSONObject(OWM_WEATHER_WIND);

        double pressure = main.optDouble(OWM_PRESSURE);
        int humidity = main.getInt(OWM_HUMIDITY);
        double windSpeed = wind.getDouble(OWM_WINDSPEED);
        double windDirection = wind.getDouble(OWM_WIND_DIRECTION);
        //  Temperatures are sent by Open Weather Map in a child object called "main".
        double max = main.getDouble(OWM_MAX);
        double min = main.getDouble(OWM_MIN);
